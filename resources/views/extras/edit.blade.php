...
@section('content')
    <div class="content">
        <div class="container-fluid">
            <div class="row">
                <div class="col-md-12">
                    <form method="post" action="{{ url('update-extra/'.$extra->id) }}" enctype="multipart/form-data" autocomplete="off" class="form-horizontal">
                        @csrf
                        @method('post')

                        <div class="card ">
                            <div class="card-header card-header-primary">
                                <h4 class="card-title">{{ __('Update Menu Item Extra') }}</h4>
                                      ...
                                      <span class="input-group-text">
                                          <i class="material-icons">folder_special</i>
                                                ...
                                                <input id="name" type="text" name="name" class="form-control"
                                                       placeholder="{{ __('Item Name...') }}" value="{{ $extra->name }}" required>
                                            </div>
                                            @if ($errors->has('name'))
                                      ...
                                      <span class="input-group-text">
                                        <i class="material-icons">attach_money</i>
                                                ...
                                                <input type="number" step="0.01" name="price" class="form-control"
                                                       placeholder="{{ __('Item Price...') }}" value="{{ $extra->price }}" required>
                                            </div>
                                            @if ($errors->has('price'))
                  ...
                  <span class="input-group-text">
                      <i class="material-icons">assignment</i>
                                            ...
                                            <textarea name="description" placeholder="Item Description" class="form-control" rows="2">{{$extra->description}}</textarea>
